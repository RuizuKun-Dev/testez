...
return{
    ['run expect-extend e2e'] = function()
        TestEZ.run(script:FindFirstChild('expect-extend'), function(results)
            assert(#results.errors == 0, 'Expected no errors, got ' .. tostring(results.errors[1]) .. ' plus ' .. tostring(#results.errors - 1) .. ' more.')
        end)
    end,
    ['run afterEach e2e with error in it block'] = function()
        TestEZ.run(script:FindFirstChild('afterEachAfterFailure'):FindFirstChild('errorInItBlock'), function(
            results
        )
            ...
            assert(cleaningUpError ~= nil, 
[[Expected 'While cleaning up...' msg to be shown when afterEach fails after it fails]])
        end)
    end,
    ['run afterEach e2e with failure in it block'] = function()
        TestEZ.run(script:FindFirstChild('afterEachAfterFailure'):FindFirstChild('failureInItBlock'), function(
            results
        )
            assert(#results.errors == 1, 'Expected one error, got ' .. tostring(#results.errors))
...
[[Expected 'While cleaning up...' msg to be shown when afterEach fails after it fails]])
    ...
    ['run afterEach e2e with failure in afterEach block'] = function()
        TestEZ.run(script:FindFirstChild('afterEachAfterFailure'):FindFirstChild('errorInAfterEachBlock'), function(
            results
        )
            assert(#results.errors == 1, 'Expected one error, got ' .. tostring(#results.errors))
            ...
            assert(cleaningUpError == nil, 
[[Expected 'While cleaning up...' msg to not be shown when only afterEach fails]])
        end)
    end,
