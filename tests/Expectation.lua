...
[[Expected function to throw an error containing "foo", but it did not.]]), ('Error message does not match:\n%s\n'):format(message))
    end,
    [
...
[[it should fail if a throwing function is expected to never throw a substring of the message]]] = function(
    ...
    ["it should fail if types don't match"] = function()
        ...
        assert(not success, 'should fail')
        assert(message:match(
[[Expected value of type "string", got value "5" of type number]]), ('Error message does not match:\n%s\n'):format(message))
    end,
    ...
    ['similar numbers should be near'] = function()
        ...
        local success = pcall(function()
            expect:near(0.1111111111111111)
        end)
    ...
    ['numbers outside the default limit should not be near'] = function()
        ...
        assert(not success, 'should fail')
        assert(message:match(
[[Expected value to be near %d+.%d+ %(within %d+.%d+%) but got %d+.%d+ instead]]), ('Error message does not match:\n%s\n'):format(message))
    end,
    ['near should respect limit argument'] = function()
        local expect = Expectation.new(0.1)
        local success = pcall(function()
            expect:near(0.1111111111111111, 0.1)
        end)
