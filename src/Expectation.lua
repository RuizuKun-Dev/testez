...
function Expectation:a(typeName)
    ...
    local message = formatMessage(self.successCondition, ('Expected value of type %q, got value %q of type %s'):format(typeName, tostring(self.value), type(self.value)), ('Expected value not of type %q, got value %q of type %s'):format(typeName, tostring(self.value), type(self.value)))
...
function Expectation:ok()
    ...
    local message = formatMessage(self.successCondition, ('Expected value %q to be non-nil'):format(tostring(self.value)), ('Expected value %q to be nil'):format(tostring(self.value)))
...
function Expectation:equal(otherValue)
    ...
    local message = formatMessage(self.successCondition, ('Expected value %q (%s), got %q (%s) instead'):format(tostring(otherValue), type(otherValue), tostring(self.value), type(self.value)), ('Expected anything but value %q (%s)'):format(tostring(otherValue), type(otherValue)))
...
function Expectation:near(otherValue, limit)
    ...
    local message = formatMessage(self.successCondition, ('Expected value to be near %f (within %f) but got %f instead'):format(otherValue, limit, self.value), (
...
[[Expected function to throw an error containing %q, but it %s]]):format(messageSubstring, err and ('threw: %s'):format(err) or 'did not throw.'), (
[[Expected function to never throw an error containing %q, but it threw: %s]]):format(messageSubstring, tostring(err)))
    else
        message = formatMessage(self.successCondition, 'Expected function to throw an error, but it did not throw.', ('Expected function to succeed, but it threw an error: %s'):format(tostring(err)))
    end
